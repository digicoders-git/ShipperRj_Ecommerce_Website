                    ...
                    @foreach($offers as $offer)
                        <tr class="border-bottom border-white border-opacity-5">
                            ...
                            <td>
                                ...
                                @if($isActive)
                                    <span
                                        class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-2 py-1">Running</span>
                                @elseif($today < $offer->start_date)
                                    <span
                                        class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 px-2 py-1">Upcoming</span>
                                @else
                                    <span
                                        class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-2 py-1">Expired/Inactive</span>
                                @endif
                            </td>
                            <td class="text-end">
                        ...
                        <form action="{{ route('admin.offers.update', $offer->id) }}" method="POST">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" name="discount_percentage" class="form-control glass-input"
                                        value="{{ $offer->discount_percentage }}" required min="1" max="100">
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label text-secondary small">Start Date</label>
                                    <input type="date" name="start_date" class="form-control glass-input"
                                        value="{{ $offer->start_date }}" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label text-secondary small">End Date</label>
                                    <input type="date" name="end_date" class="form-control glass-input"
