...
<div class="mb-4 d-flex align-items-center justify-content-between animation-fade-in">
    ...
    <a href="{{ route('admin.users.index') }}" class="btn btn-sm btn-white border px-4 rounded-pill shadow-sm transition-all hover-translate-x-left">
        ...
        <div class="glass-card h-100 p-0 overflow-hidden shadow-sm border-0 border-radius-24">
            ...
            <div class="p-4 text-center bg-white border-bottom position-relative">
                <div class="profile-photo-wrapper mb-3 position-relative d-inline-block">
                    ...
                    @endphp
                    @if($profile_photo)
                        <img src="{{ $profile_photo }}" class="rounded-circle shadow-sm p-1 border border-primary border-opacity-25" style="width: 120px; height: 120px; object-fit: cover;">
                    @else
                        <div class="rounded-circle bg-primary bg-opacity-10 d-flex align-items-center justify-content-center text-primary display-4 fw-bold shadow-sm" style="width: 120px; height: 120px;">
                            {{ substr($user->name, 0, 1) }}
                        </div>
                    @endif
                    <div class="position-absolute bottom-0 end-0 mb-1 me-1 shadow-sm">
                        @if($user->is_blocked)
                            <span class="badge bg-danger rounded-circle p-2 border border-white border-2" title="User Blocked">
                                <i class="bi bi-lock-fill"></i>
                            </span>
                        @else
                            <span class="badge bg-success rounded-circle p-2 border border-white border-2" title="User Active">
                ...
                <div class="d-flex justify-content-center gap-2">
                    @if($user->is_blocked)
                        <span class="badge bg-danger bg-opacity-10 text-danger rounded-pill px-3 py-2 fw-bold small">ACCESS BLOCKED</span>
                    @else
                        <span class="badge bg-success bg-opacity-10 text-success rounded-pill px-3 py-2 fw-bold small">ACTIVE ACCOUNT</span>
                    @endif
                </div>
            </div>
            ...
            <div class="p-4 bg-light bg-opacity-50">
                <div class="row g-2 mb-4">
                    <div class="col-6">
                        <div class="stat-card p-3 bg-white rounded-4 border border-dark border-opacity-5 text-center shadow-sm">
                    ...
                    <div class="row g-2">
                        <div class="col-6">
                            <div class="p-3 bg-white rounded-4 border border-dark border-opacity-5 shadow-sm d-flex align-items-center h-100">
                                <div class="icon-box me-2"><i class="bi bi-person-badge text-primary"></i></div>
                                <div class="overflow-hidden">
                                    <label class="xx-small text-secondary fw-bold uppercase d-block">User ID</label>
                                    <span class="text-dark x-small fw-bold text-truncate d-block">{{ $user->id }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="col-6">
                            <div class="p-3 bg-white rounded-4 border border-dark border-opacity-5 shadow-sm d-flex align-items-center h-100">
                                <div class="icon-box me-2"><i class="bi bi-telephone text-primary"></i></div>
                                <div class="overflow-hidden">
                                    <label class="xx-small text-secondary fw-bold uppercase d-block">Phone</label>
                                    <span class="text-dark x-small fw-bold text-truncate d-block">{{ $user->mobile ?? 'N/A' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    
                    <div class="p-3 bg-white rounded-4 border border-dark border-opacity-5 shadow-sm d-flex align-items-center overflow-hidden">
            ...
            <div class="p-4 bg-white border-top">
                <form action="{{ route('admin.users.toggle-block', $user->id) }}" method="POST" class="w-100">
                    @csrf
                    @if($user->is_blocked)
                        <button type="submit" class="btn btn-success w-100 py-3 rounded-pill fw-bold shadow-sm transition-all hover-scale border-0 btn-block-toggle" data-status="unblock">
                            <i class="bi bi-unlock-fill me-2"></i> ACTIVATE USER
                        </button>
                    @else
                        <button type="submit" class="btn btn-danger w-100 py-3 rounded-pill fw-bold shadow-sm transition-all hover-scale border-0 btn-block-toggle" data-status="block">
            ...
            <div class="bg-white p-2 border-bottom">
                <ul class="nav nav-pills custom-pills-new gap-2" id="userTabs" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-link active px-4 py-2 rounded-pill small fw-bold" id="orders-tab" data-bs-toggle="tab" data-bs-target="#orders-pane" type="button" role="tab">Orders List</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link px-4 py-2 rounded-pill small fw-bold" id="wallet-tab" data-bs-toggle="tab" data-bs-target="#wallet-pane" type="button" role="tab">Wallet Actions</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link px-4 py-2 rounded-pill small fw-bold" id="addresses-tab" data-bs-toggle="tab" data-bs-target="#addresses-pane" type="button" role="tab">Saved Addresses</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-link px-4 py-2 rounded-pill small fw-bold" id="wishlist-tab" data-bs-toggle="tab" data-bs-target="#wishlist-pane" type="button" role="tab">Cart & Items</button>
                    </li>
                </ul>
            </div>
            ...
            <div class="tab-content" id="userTabsContent" style="max-height: 700px; overflow-y: auto;">
                            ...
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-bottom transition-all">
                                        ...
                                        <td class="small fw-bold text-dark">₹{{ number_format($order->total_amount, 2) }}</td>
                                        <td>
                                            <span class="badge bg-primary bg-opacity-10 text-white rounded-pill px-3 py-1 fw-bold" style="font-size: 0.65rem;">{{ strtoupper($order->order_status) }}</span>
                                        </td>
                                        <td class="pe-3 text-end">
                                            <a href="{{ route('admin.orders.show', $order->id) }}" class="btn btn-sm btn-outline-primary p-1 rounded-circle">
                            ...
                            <table class="table table-hover align-middle">
                                ...
                                <tbody>
                                    ...
                                    <tr class="border-bottom">
                                        ...
                                        <td>
                                            <span class="{{ $tx->amount > 0 ? 'text-success' : 'text-danger' }} fw-bold px-3 py-1 rounded-pill bg-light border shadow-sm small">
                ...
                <div class="tab-pane fade p-4" id="addresses-pane" role="tabpanel">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            <div class="p-4 rounded-4 bg-light border border-opacity-10 h-100 shadow-sm transition-all hover-translate-y">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <span class="badge bg-primary text-white rounded-pill px-3 py-1 fw-bold x-small">{{ strtoupper($address->type) }}</span>
                                    @if($address->is_default)
                                        <span class="text-primary xx-small fw-bold"><i class="bi bi-star-fill me-1"></i>Default</span>
                                    @endif
                                </div>
                                <h6 class="text-dark fw-bold mb-1">{{ $address->name }}</h6>
                                <p class="text-secondary small fw-bold mb-3"><i class="bi bi-phone me-1"></i>{{ $address->mobile }}</p>
                                <div class="p-3 bg-white rounded-3 border">
                                    <p class="text-dark small mb-0 opacity-75">
                                        {{ $address->address_line }}, {{ $address->landmark ? $address->landmark.',' : '' }} 
                                        <span class="d-block mt-2 fw-bold text-primary">{{ $address->city }}, {{ $address->state }} - {{ $address->pincode }}</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row g-4">
                        <div class="col-md-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="p-3 bg-white rounded-4 border d-flex align-items-center gap-3 shadow-sm hover-translate-y transition-all">
                                    <img src="{{ asset($cart->product->image) }}" class="rounded-3 border" style="width: 50px; height: 50px; object-fit: cover;">
                                    <div class="flex-grow-1 overflow-hidden">
                                        <div class="text-dark fw-bold small text-truncate">{{ $cart->product->name }}</div>
                                        <div class="text-secondary xx-small uppercase fw-bold">Qty: {{ $cart->quantity }} • ₹{{ number_format($cart->product->selling_price, 2) }}</div>
                                    </div>
                                    <div class="text-primary fw-bold">₹{{ number_format($cart->product->selling_price * $cart->quantity, 2) }}</div>
                                </div>
                                @empty
                                <div class="bg-light py-5 text-center text-muted small border rounded-4">Cart is empty</div>
                                @endforelse
                            </div>
                        </div>
                        <div class="col-md-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="p-3 bg-white rounded-4 border d-flex align-items-center gap-3 shadow-sm hover-translate-y transition-all">
                                    <img src="{{ asset($wish->product->image) }}" class="rounded-3 border" style="width: 50px; height: 50px; object-fit: cover;">
                                    <div class="flex-grow-1 overflow-hidden pe-2">
                                        <div class="text-dark fw-bold small text-truncate">{{ $wish->product->name }}</div>
                                        <div class="text-primary fw-bold small mt-1">₹{{ number_format($wish->product->selling_price, 2) }}</div>
                                    </div>
                                    <a href="{{ route('admin.products.show', $wish->product->id) }}" class="btn btn-sm btn-dark rounded-pill px-3">View</a>
                                </div>
                                @empty
                                <div class="bg-light py-5 text-center text-muted small border rounded-4">Wishlist is empty</div>
                                @endforelse
                            </div>
                        </div>
                    </div>
...
<style>
    ...
    .hover-translate-x-left:hover { transform: translateX(-4px); }
