                ...
                <tbody>
                    @foreach($orders as $order)
                        <tr class="border-bottom border-white border-opacity-5">
                            ...
                            <td>
                                <span
                                    class="badge bg-{{ $order->payment_status == 'paid' ? 'success' : 'warning' }} bg-opacity-10 text-{{ $order->payment_status == 'paid' ? 'success' : 'warning' }} border border-{{ $order->payment_status == 'paid' ? 'success' : 'warning' }} border-opacity-25 px-2 py-1 x-small">
                            ...
                            <td>
                                <span
                                    class="badge bg-info bg-opacity-10 text-info border border-info border-opacity-25 px-2 py-1 x-small">
                    ...
                    <div class="modal-body p-4 pt-4">
                        <form action="{{ route('admin.orders.update', $order->id) }}" method="POST">
                            ...
                            @if($order->cancel_reason || $order->return_reason)
                                <div class="glass-card mb-4 bg-light border-0 p-3 rounded-4">
                                    @if($order->cancel_reason)
                                        <div class="mb-2">
                                            <label class="xx-small text-danger fw-black uppercase mb-1 d-block">Cancellation
                                                Intelligence</label>
                                    ...
                                    @if($order->return_reason)
                                        <div>
                                            <label class="xx-small text-warning fw-black uppercase mb-1 d-block">Return
                                                Intelligence</label>
                                            <p class="x-small text-dark mb-0 italic">"{{ $order->return_reason }}"</p>
                                            @if($order->return_status)
                                                <span
                                                    class="badge bg-warning bg-opacity-20 text-warning xx-small mt-1 px-2 py-1 rounded-pill">{{ strtoupper($order->return_status) }}</span>
                                            @endif
                                        </div>
                                    @endif
                                </div>
                            ...
                            @if($order->refund_status)
                                <div
                                    class="mb-4 d-flex align-items-center justify-content-between p-3 bg-primary bg-opacity-5 rounded-4 border border-primary border-opacity-10">
                                    <div>
                                        <label class="xx-small text-primary fw-black uppercase d-block mb-1">Financial
                                            Settlement</label>
                                        <span class="x-small fw-bold text-dark">REFUND:
                                            {{ strtoupper($order->refund_status) }}</span>
                                    </div>
                                    <span
                                        class="h6 mb-0 fw-black text-primary">₹{{ number_format($order->refund_amount, 2) }}</span>
                                </div>
                            @endif
