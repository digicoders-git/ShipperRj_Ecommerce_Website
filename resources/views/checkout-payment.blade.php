            ...
            <div class="card border-0 shadow-premium p-5 rounded-5 animate-fade-in">
                ...
                <div class="bg-light p-4 rounded-4 mb-4 text-start">
                    <div class="d-flex justify-content-between mb-2">
                        ...
                        <span class="fw-black h4 mb-0 text-primary">₹{{ number_format($order->prepaid_amount) }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center mt-3 pt-3 border-top border-white border-opacity-50">
                        <div class="xx-small text-muted uppercase tracking-widest fw-black">Wallet Balance: <b class="text-dark">₹{{ number_format(Auth::user()->wallet_balance) }}</b></div>
                        @if(Auth::user()->wallet_balance >= $order->prepaid_amount)
                            <span class="badge bg-success-soft text-success xx-small fw-black uppercase">Sufficient Balance</span>
                        @else
                            <span class="badge bg-danger-soft text-danger xx-small fw-black uppercase">Insufficient Balance</span>
                        @endif
                    </div>
                </div>

                <div class="d-grid gap-3">
                    <button id="rzp-button1" class="btn btn-primary w-100 py-3 rounded-pill fw-black transform-transition hover-scale shadow-lg border-0" style="background-color: #3399cc !important;">
                        <i class="bi bi-credit-card me-2"></i> PAY VIA RAZORPAY
                    </button>

                    <button id="walletPayBtn" class="btn btn-dark w-100 py-3 rounded-pill fw-black transform-transition hover-scale shadow-lg border-0" 
...
<script>
    ...
    document.getElementById('walletPayBtn').onclick = function(e){
        if(confirm('Are you sure you want to pay ₹{{ number_format($order->prepaid_amount) }} from your wallet?')) {
            const btn = this;
            btn.disabled = true;
