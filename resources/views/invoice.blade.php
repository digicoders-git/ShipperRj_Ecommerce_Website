...
<body>
    ...
    <div class="no-print container py-4 d-flex justify-content-center gap-3">
        ...
        <a href="{{ url('/order-success?order_id=' . $order->id) }}"
            class="btn btn-link text-muted fw-bold text-decoration-none py-2">
    ...
    <div class="invoice-wrapper">
        <div class="invoice-box" id="invoiceContent">
                ...
                <div class="col-5 text-end">
                    ...
                    <div class="d-flex flex-column align-items-end gap-1">
                        ...
                        <p class="mb-0 text-secondary small">DATE: <span
                                class="text-dark fw-medium">{{ $order->created_at->format('d M, Y | H:i') }}</span></p>
                    </div>
                </div>
            </div>
            ...
            <div class="row mb-5 position-relative">
                <div class="col-6">
                    <div class="p-4 rounded-4 bg-premium-light border border-white h-100">
                        ...
                        <div class="d-flex align-items-center gap-2 small text-dark fw-bold mb-2">
                        ...
                        @if($order->has_gst && $order->gst_number)
                            <div class="mt-3 pt-3 border-top border-light-subtle">
                                <p class="xx-small text-secondary fw-bold uppercase tracking-wider mb-1"
                                    style="font-size: 0.65rem; color: var(--premium-orange) !important;">GST INVOICE DETAILS
                                </p>
                                @if($order->gst_company)
                                    <p class="mb-0 small fw-bold text-dark">{{ $order->gst_company }}</p>
                                @endif
                                <p class="mb-0 small fw-medium text-secondary">GSTIN: <span
                                        class="text-dark fw-bold">{{ $order->gst_number }}</span></p>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="p-4 rounded-4 border border-light h-100 d-flex flex-column justify-content-between">
                        <div class="row g-3">
                            ...
                            <div class="col-6">
                                ...
                                <span
                                    class="text-dark fw-black small text-uppercase">{{ $order->payment_method }}</span>
                            </div>
                        </div>
                        <div class="mt-4 pt-3 border-top">
                            ...
                            <div class="d-flex align-items-center gap-3">
                                <div class="qr-placeholder d-flex align-items-center justify-content-center">
                                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data={{ urlencode(url('/order-track?number=' . $order->order_number)) }}"
                                        class="img-fluid" alt="QR">
                                </div>
                                <div class="small text-secondary opacity-75 fw-medium">Scan to verify order<br>integrity
                                    online.</div>
                            </div>
                        </div>
                    </div>
                        ...
                        @foreach($order->orderItems as $index => $item)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($item->product->sku)
                                        <div class="xx-small text-muted mt-1 opacity-75 fw-bold">SKU-ID:
                                            {{ $item->product->sku }}
                                        </div>
                                    @endif
                                </td>
                                ...
                                <td class="text-end fw-black text-dark outfit x-small">
                ...
                <div class="col-md-6">
                    <div class="p-4 rounded-4 bg-premium-light mt-4">
                        ...
                        <p class="small text-secondary mb-0 lh-lg" style="font-size: 0.75rem;">
                            This is an authentic tax invoice for your purchase. Please keep this for any warranty
                            claims.
                            In case of any discrepancy, please contact our support team within 24 hours of receiving the
                            shipment.
                        </p>
                    </div>
                </div>
                <div class="col-md-5">
                    <div class="summary-box">
                        <div class="d-flex justify-content-between mb-3 align-items-center">
                            ...
                            <span
                                class="fw-black text-dark outfit x-small">₹{{ number_format($order->total_amount - $order->shipping_amount - $order->gst_amount + $order->coupon_discount) }}</span>
                        </div>
                        ...
                        @if($order->payment_method == 'cod')
                            <div class="mt-4 pt-3 border-top border-2 border-white">
                                <div class="d-flex justify-content-between mb-2">
                                    <span class="text-secondary x-small fw-bold opacity-75 uppercase">Paid Online
                                        (Advance)</span>
                                    <span
                                        class="fw-bold text-success x-small">+₹{{ number_format($order->prepaid_amount) }}</span>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <span class="text-dark small fw-black uppercase tracking-tighter">Amount Due on
                                        COD</span>
                                    <span
                                        class="fw-black text-orange outfit">₹{{ number_format($order->cod_amount) }}</span>
                                </div>
                            </div>
                        @else
            ...
            <!-- Footer Section -->
            <div class="footer-note position-relative text-center">
                <p class="small text-secondary mb-0 lh-base" style="font-size: 0.75rem;">
                    <span class="fw-bold text-dark">Note:</span> This is a system-generated e-Invoice/e-Receipt and is
                    valid without a physical or digital signature as per applicable electronic invoicing regulations.
                </p>
            </div>
        </div>
    </div>
