                ...
                <tr class="border-bottom border-white border-opacity-5">
                    <td>
                        <div class="d-flex align-items-center">
                            <div class="rounded-circle bg-primary bg-opacity-20 d-flex align-items-center justify-content-center text-white small fw-bold me-3" style="width: 40px; height: 40px;">{{ substr($subadmin->name, 0, 1) }}</div>
                            <div>
                    ...
                    <td>
                        <span class="badge bg-white bg-opacity-5 text-secondary border border-white border-opacity-10 px-3 py-2 rounded-3 x-small">
                            <i class="bi bi-person me-1"></i> {{ $subadmin->username }}
                        </span>
                    </td>
                    <td>
                        <span class="badge bg-primary bg-opacity-10 text-light-soft border border-primary border-opacity-25 px-3 py-2 rounded-3 x-small">
                    ...
                    <td>
                        <form action="{{ route('admin.subadmins.toggle-status', $subadmin) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-sm border-0 p-0">
                                @if($subadmin->status)
                                    <span class="badge bg-success bg-opacity-10 text-success border border-success border-opacity-25 px-3 py-2 rounded-3 x-small">Active</span>
                                @else
                                    <span class="badge bg-danger bg-opacity-10 text-danger border border-danger border-opacity-25 px-3 py-2 rounded-3 x-small">Inactive</span>
                                @endif
                            </button>
                        </form>
                    </td>
                    <td>
                        @if($subadmin->last_login_at)
                            ...
                            <div class="xx-small text-secondary opacity-50">{{ $subadmin->last_login_at->format('h:i A') }}</div>
                        @else
                    ...
                    <td class="text-end">
                        <div class="d-flex justify-content-end gap-2">
                            <button type="button" class="btn btn-sm btn-outline-light border-0 bg-white bg-opacity-5" 
                                onclick="viewSubAdmin({{ json_encode($subadmin) }}, {{ json_encode($subadmin->permissions ?? []) }})" title="View Details">
                                <i class="bi bi-eye text-info"></i>
                            </button>

                            <a href="{{ route('admin.subadmins.edit', $subadmin) }}" class="btn btn-sm btn-outline-light border-0 bg-white bg-opacity-5" title="Edit Account">
                                <i class="bi bi-pencil-square text-primary"></i>
                            </a>

                            <form action="{{ route('admin.subadmins.destroy', $subadmin) }}" method="POST" class="d-inline border-0">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline-light border-0 bg-white bg-opacity-5 btn-delete" title="Delete Account">
...
<div class="modal fade" id="subAdminViewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content overflow-hidden border-0 shadow-lg rounded-4">
            <div class="modal-header border-0 p-4" style="background: linear-gradient(135deg, #f0ca84ff 0%, #f7c048ff 100%);">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        ...
                        <p class="text-white opacity-50 small mb-0 font-monospace" id="modalSubAdminUsername">admin_user</p>
                    </div>
                </div>
                ...
                <div class="row g-4">
                    ...
                    <div class="col-md-6">
                        <div class="p-4 bg-white rounded-4 shadow-sm h-100 border border-dark border-opacity-5">
                            ...
                            <div class="mb-0">
                                <label class="xx-small text-muted fw-bold text-uppercase d-block mb-1">Phone/Mobile</label>
                    ...
                    <div class="col-md-6">
                        <div class="p-4 bg-white rounded-4 shadow-sm h-100 border border-dark border-opacity-5">
                            ...
                            <div class="mb-3">
                                <label class="xx-small text-muted fw-bold text-uppercase d-block mb-1">Current Password</label>
                                <div class="d-flex align-items-center">
                                    <span class="fw-bold text-muted font-monospace bg-light px-3 py-1 rounded small border">
                                        ••••••••••••
                                    </span>
                                    <span class="badge bg-danger-soft text-danger border-0 ms-2 xx-small fw-black text-uppercase">Encrypted</span>
                                </div>
                            </div>
                            <div class="mb-0">
                                <label class="xx-small text-muted fw-bold text-uppercase d-block mb-1">Created On</label>
            ...
            <div class="modal-footer border-0 p-4 pt-0 bg-light">
                <button type="button" class="btn btn-secondary rounded-pill px-5 fw-bold" data-bs-dismiss="modal">Close Panel</button>
            </div>
        </div>
    </div>
</div>
