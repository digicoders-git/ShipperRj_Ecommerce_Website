                ...
                <tbody>
                    @foreach($subCategories as $subCategory)
                        <tr class="border-bottom border-white border-opacity-5">
                            ...
                            <td class="small">{{ $subCategory->id }}</td>
                                ...
                                <div class="d-flex justify-content-end gap-2">
                                    @if($isAdmin || ($subAdmin && $subAdmin->hasPermission('sub_categories_edit')))
                                        <button class="btn btn-sm btn-outline-light border-0 bg-white bg-opacity-5"
                                            data-bs-toggle="modal" data-bs-target="#editSubCategoryModal{{ $subCategory->id }}">
                                            <i class="bi bi-pencil-square text-primary"></i>
                                        </button>
                                    @endif

                                    @if($isAdmin || ($subAdmin && $subAdmin->hasPermission('sub_categories_delete')))
                                        <form action="{{ route('admin.sub-categories.destroy', $subCategory->id) }}" method="POST"
                                            class="delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button"
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="description" class="form-control glass-input"
                                        rows="2">{{ $subCategory->description }}</textarea>
                                </div>
                                <div class="col-md-12 mb-3">
                    ...
                    <form action="{{ route('admin.sub-categories.store') }}" method="POST">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-12 mb-3">
                                <label class="form-label text-secondary small">Description</label>
                                <textarea name="description" class="form-control glass-input"
                                    placeholder="Enter sub category description" rows="2"></textarea>
                            </div>
                            <div class="col-md-12 mb-3">
                                <label class="form-label text-secondary small">Status</label>
                                <select name="status" class="form-select glass-input">
