    ...
    <div>
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('admin.orders.index') }}" class="text-primary text-decoration-none small fw-bold">Orders</a></li>
                <li class="breadcrumb-item active small fw-bold text-secondary" aria-current="page">Details</li>
            </ol>
        </nav>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('admin.orders.index') }}" class="btn btn-light border bg-white px-3 shadow-sm rounded-3 small fw-bold transition-all">
            <i class="bi bi-arrow-left me-1 text-secondary"></i> <span class="text-secondary">Back to List</span>
        </a>
        <a href="{{ route('admin.orders.invoice', $order->id) }}" target="_blank" class="btn btn-info px-4 py-2 shadow-sm rounded-3 border-0 transition-all hover-scale fw-bold text-white">
            <i class="bi bi-file-earmark-pdf me-2"></i> Generate Invoice
        </a>
        <button class="btn btn-premium px-4 py-2 shadow-lg rounded-3 border-0 transition-all hover-scale" data-bs-toggle="modal" data-bs-target="#updateOrderModal{{ $order->id }}">
    ...
    <div class="col-lg-8">
        ...
        <div class="row g-3 mb-4 text-dark">
            <div class="col-sm-4">
                <div class="card border-0 shadow-sm rounded-4 p-3 h-100 bg-white border-start border-4 border-primary">
                    <p class="xx-small text-secondary fw-black uppercase tracking-widest mb-1 opacity-75">Current Status</p>
                    <div class="d-flex align-items-center gap-2">
                        ...
                        @endphp
                        <span class="badge bg-{{ $color }} bg-opacity-10 text-{{ $color }} px-3 py-1 rounded-pill x-small fw-black uppercase tracking-widest">
            ...
            <div class="col-sm-4">
                <div class="card border-0 shadow-sm rounded-4 p-3 h-100 bg-white border-start border-4 border-info">
                    <p class="xx-small text-secondary fw-black uppercase tracking-widest mb-1 opacity-75">Payment Method</p>
            ...
            <div class="col-sm-4">
                <div class="card border-0 shadow-sm rounded-4 p-3 h-100 bg-white border-start border-4 border-success">
                    <p class="xx-small text-secondary fw-black uppercase tracking-widest mb-1 opacity-75">Date & Time</p>
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4 bg-white">
            <div class="card-header bg-white border-0 p-4 pb-0 d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-secondary bg-opacity-10 text-secondary border border-dark border-opacity-10 px-2 py-1 xx-small fw-bold">
                                ...
                                <td class="ps-4">
                                    <div class="d-flex align-items-center gap-3 py-2">
                                        <div class="position-relative">
                                            <div class="rounded-3 border border-dark border-opacity-10 p-1 bg-white" style="width: 55px; height: 55px;">
                                                <img src="{{ asset($item->product->image) }}" class="img-fluid rounded-2 h-100 w-100 object-fit-contain" alt="">
                                            </div>
                                        </div>
                                        <div>
                                            <div class="small fw-black text-dark mb-0 line-clamp-1">{{ $item->product->name }}</div>
                                            <div class="xx-small text-secondary tracking-widest uppercase opacity-75">SKU: {{ $item->product->sku ?? 'N/A' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="text-center small fw-bold text-dark">₹{{ number_format($item->price, 2) }}</td>
                                <td class="text-center">
                                    <span class="badge bg-light text-dark px-3 py-1 rounded-pill small border border-dark border-opacity-10">{{ $item->quantity }}</span>
                                </td>
                                <td class="pe-4 text-end small fw-black text-dark">₹{{ number_format($item->price * $item->quantity, 2) }}</td>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
                ...
                <div class="row justify-content-end">
                    <div class="col-md-5">
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <span class="small fw-bold text-dark">₹{{ number_format($order->total_amount - $order->shipping_amount + $order->coupon_discount, 2) }}</span>
                        </div>
                        <div class="d-flex justify-content-between mb-2">
                            <span class="xx-small text-secondary fw-black uppercase tracking-widest">Shipping Charges</span>
                            <span class="small fw-bold text-dark">₹{{ number_format($order->shipping_amount, 2) }}</span>
                        </div>
                        @if($order->coupon_discount > 0)
                        <div class="d-flex justify-content-between mb-2">
                            <span class="xx-small text-secondary fw-black uppercase tracking-widest">Coupon Discount ({{ $order->coupon_code }})</span>
                            <span class="small fw-bold text-success">-₹{{ number_format($order->coupon_discount, 2) }}</span>
                        </div>
                        @endif
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            <span class="xx-small text-secondary fw-black uppercase tracking-widest">Advanced Paid</span>
                            <span class="small fw-bold text-primary">₹{{ number_format($order->prepaid_amount, 2) }}</span>
                        </div>
        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white mb-4">
            ...
            <div class="card-body p-4 pt-1">
                <div class="timeline position-relative ps-4 py-3">
                    <div class="timeline-line position-absolute start-0 top-0 bottom-0 ms-1 border-start border-2 border-light border-opacity-75"></div>
                    ...
                    <div class="timeline-item position-relative mb-4 last-no-border">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-1">
                            ...
                            <span class="xx-small text-secondary opacity-75 font-monospace">{{ $track->created_at->format('d M, Y | h:i A') }}</span>
                        </div>
                        <p class="small text-secondary mb-0 p-3 rounded-3 bg-light bg-opacity-50 border border-light">{{ $track->message }}</p>
                    </div>
                    @empty
        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white mb-4">
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-center gap-3 mb-4">
                    <div class="rounded-4 bg-primary bg-opacity-10 text-primary d-flex align-items-center justify-content-center border-0" style="width: 55px; height: 55px;">
                ...
                <div class="mb-4">
                    <h6 class="xx-small text-secondary fw-black uppercase tracking-widest mb-3 opacity-75">Shipping Address</h6>
        ...
        <div class="card border-0 shadow-sm rounded-4 bg-white mb-4">
            ...
            <div class="card-body p-4">
                <div class="d-flex justify-content-between mb-3 border-bottom border-light pb-2">
                    ...
                    <span class="badge bg-{{ $order->payment_status == 'paid' ? 'success' : 'warning' }} bg-opacity-10 text-{{ $order->payment_status == 'paid' ? 'success' : 'warning' }} px-3 py-1 rounded-pill xx-small fw-black uppercase">
                ...
                <div class="mb-3">
                    <span class="xx-small text-secondary fw-black uppercase tracking-widest d-block mb-1">Gateway Reference</span>
                    <div class="small fw-black text-primary bg-primary bg-opacity-5 p-2 rounded border border-primary border-opacity-10 font-monospace">
