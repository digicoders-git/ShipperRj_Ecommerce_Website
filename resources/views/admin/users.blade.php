                ...
                <tr class="border-bottom border-white border-opacity-5">
                    ...
                    <td>
                        <div class="d-flex align-items-center">
                            ...
                            @endphp
                            @if($profile_photo)
                                <img src="{{ $profile_photo }}" class="rounded-circle me-2" style="width: 35px; height: 35px; object-fit: cover;">
                            @else
                                <div class="rounded-circle bg-primary bg-opacity-20 d-flex align-items-center justify-content-center text-white small fw-bold me-2" style="width: 35px; height: 35px;">{{ substr($user->name, 0, 1) }}</div>
                            @endif
                            <div>
                    ...
                    <td class="small text-secondary fw-bold" style="max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: wrap;">
                        @if($user->address)
                    ...
                    <td>
                        <span class="text-success fw-bold small">₹{{ number_format($user->wallet_balance ?? 0, 2) }}</span>
                    </td>
                    <td>
                        @if($user->is_blocked)
                            <span class="badge bg-danger bg-opacity-10 text-danger rounded-pill x-small px-3">Blocked</span>
                        @else
                            <span class="badge bg-success bg-opacity-10 text-success rounded-pill x-small px-3">Active</span>
                        @endif
                    </td>
                    <td class="small">{{ $user->created_at ? $user->created_at->format('d M, Y') : 'N/A' }}</td>
                    <td class="text-end">
                        <div class="d-flex justify-content-end gap-2">
                            <a href="{{ route('admin.users.show', $user->id) }}" class="btn btn-sm btn-outline-light border-0 bg-white bg-opacity-5" title="View Details">
                                <i class="bi bi-eye text-primary"></i>
                            </a>
                            <form action="{{ route('admin.users.toggle-block', $user->id) }}" method="POST" class="d-inline">
                                @csrf
                                <button type="submit" class="btn btn-sm btn-outline-light border-0 bg-white bg-opacity-5 btn-block-toggle" 
                            ...
                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline-light border-0 bg-white bg-opacity-5 btn-delete" title="Delete User">
